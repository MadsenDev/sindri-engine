...
- `patch_component` data fields: Sprite supports texture_path, width, height, flip_x, flip_y, color [r,g,b,a]; AnimatedSprite supports texture_path, cols, rows, width, height, flip_x, flip_y, tint, margin, spacing, clips (array of clip objects), default_clip; Tilemap supports texture_path, tileset_cols, tileset_rows, tile_width, tile_height, map_cols, map_rows, tiles (flat u16 array, 0=empty, 1-based tileset index), solid_tiles (array of tile IDs that are solid — auto-generates static physics colliders on play), tint, margin, spacing; Collider supports width, height, offset_x, offset_y, is_trigger; PhysicsBody supports body_type, lock_rotation, linear_damping, angular_damping, collision_layer, collision_mask; Script supports path; Camera supports active, zoom, follow_entity, offset_x, offset_y, bounds_min_x, bounds_min_y, bounds_max_x, bounds_max_y, smoothing, dead_zone_width, dead_zone_height; AudioSource supports path, volume, looping, play_on_start.
