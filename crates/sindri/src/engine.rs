...
/// Main entrypoint for running a Sindri game.
pub struct Engine {
    config: EngineConfig,
}
...
impl EngineContext {
    ...
    pub fn fixed_update_alpha(&self) -> f32 {
        if self.fixed_delta_time.as_secs_f32() > 0.0 {
            (self.fixed_time_accumulator.as_secs_f32() / self.fixed_delta_time.as_secs_f32())
                .min(1.0)
        } else {
            0.0
        }
    ...
    /// Until you configure actual font files in `sindri::fonts`, this will
