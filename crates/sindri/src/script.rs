...
impl ScriptCommandBuffer {
    ...
    pub fn set_collision_layer(&mut self, entity: EntityId, layer: u8) {
        self.commands
            .push(ScriptCommand::SetCollisionLayer { entity, layer });
    }

    pub fn set_collision_mask(&mut self, entity: EntityId, layer: u8, mask: u32) {
        self.commands.push(ScriptCommand::SetCollisionMask {
            entity,
            layer,
            mask,
        });
    ...
    pub fn update_animation(&mut self, entity: EntityId, dt: f32) {
        self.commands
            .push(ScriptCommand::UpdateAnimation { entity, dt });
    }

    pub fn set_animation_playing(&mut self, entity: EntityId, playing: bool) {
        self.commands
            .push(ScriptCommand::SetAnimationPlaying { entity, playing });
    }
    ...
    pub fn set_animation_speed(&mut self, entity: EntityId, speed: f32) {
        self.commands
            .push(ScriptCommand::SetAnimationSpeed { entity, speed });
    }

    pub fn set_tilemap_tile(&mut self, entity: EntityId, x: u32, y: u32, tile_id: u32) {
        self.commands.push(ScriptCommand::SetTilemapTile {
            entity,
            x,
            y,
            tile_id,
    ...
    pub fn fill_tilemap_rect(
        ...
        self.commands.push(ScriptCommand::FillTilemapRect {
            entity,
            x,
            y,
            width,
    ...
    pub fn zoom_camera(&mut self, entity: EntityId, target: f32, speed: f32) {
        self.commands.push(ScriptCommand::ZoomCamera {
            entity,
            target,
            speed,
        });
    ...
    pub fn apply(&mut self, world: &mut World, physics: &mut PhysicsWorld) {
        ...
        for command in self.commands.drain(..) {
            match command {
                ...
                ScriptCommand::SetCollisionLayer { entity, layer } => {
                    physics.set_collision_layer(entity, layer);
                }
                ScriptCommand::SetCollisionMask {
                    entity,
                    layer,
                    mask,
                } => {
                    physics.set_collision_mask(entity, layer, mask);
                }
                ...
                ScriptCommand::SetTilemapTile {
                    entity,
                    x,
                    y,
                    tile_id,
                } => {
                    if let Some(tilemap_comp) =
                        world.get_mut::<crate::entities::TilemapComponent>(entity)
                    {
                        tilemap_comp.tilemap.set_tile(x, y, tile_id);
                    }
                }
                ScriptCommand::FillTilemapRect {
                    entity,
                    x,
                    y,
                    width,
...
impl UserData for ScriptSelf {
    fn add_methods<'lua, M: UserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        methods.add_method("input", |_, this, ()| Ok(InputFacet { input: this.input }));
        ...
        methods.add_method("set_layer", |_, this, layer: u8| {
            if let Ok(mut commands) = this.commands.lock() {
                commands.set_collision_layer(this.entity, layer);
            }
            Ok(())
        });
        methods.add_method("set_mask", |_, this, (layer, mask): (u8, u32)| {
            if let Ok(mut commands) = this.commands.lock() {
                commands.set_collision_mask(this.entity, layer, mask);
            }
            Ok(())
        });
    }
...
impl UserData for InputFacet {
    fn add_methods<'lua, M: UserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        // Returns -1.0 if `neg` is held, 1.0 if `pos` is held, 0.0 if neither
        // or both are held. Useful for movement: input:axis("A","D")
        methods.add_method("axis", |_, this, (neg, pos): (String, String)| {
            let input = unsafe { &*this.input };
            let n = parse_key(&neg)
                .map(|k| input.is_key_down(k))
                .unwrap_or(false);
            let p = parse_key(&pos)
                .map(|k| input.is_key_down(k))
                .unwrap_or(false);
        ...
        // Same as `axis` but allows both pressed simultaneously; result is
        // the sum clamped to -1..1.
        methods.add_method("axis_raw", |_, this, (neg, pos): (String, String)| {
            let input = unsafe { &*this.input };
            let n = if parse_key(&neg)
                .map(|k| input.is_key_down(k))
                .unwrap_or(false)
            {
                -1.0_f32
...
impl UserData for WorldFacet {
    fn add_methods<'lua, M: UserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        methods.add_method("despawn", |_, this, entity_raw: i64| {
            if entity_raw < 0 {
                return Err(mlua::Error::RuntimeError(
                    "Entity id must be non-negative".to_string(),
                ));
            }
        ...
        methods.add_method(
            "spawn_dynamic",
            |_, this, (position, velocity): (Vec2, Vec2)| {
                if let Ok(mut commands) = this.commands.lock() {
        ...
        methods.add_method(
            "spawn_empty",
            |_, this, (position, tag): (Option<Vec2>, Option<String>)| {
                if let Ok(mut commands) = this.commands.lock() {
                    commands.spawn(SpawnRequest {
...
impl UserData for AnimationFacet {
    fn add_methods<'lua, M: UserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        methods.add_method("current_frame_index", |_, this, ()| {
            let world = unsafe { &*this.world };
            Ok(world
                .get::<AnimatedSprite>(this.entity)
...
impl UserData for TilemapFacet {
    fn add_methods<'lua, M: UserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        methods.add_method(
            "fill_rect",
            |_, this, (x, y, w, h, tile_id): (u32, u32, u32, u32, u32)| {
                if let Ok(mut commands) = this.commands.lock() {
...
impl ScriptRuntime {
    ...
    pub fn new() -> Result<Self> {
        ...
        let vec2_func =
            lua.create_function(|_, (x, y): (f64, f64)| Ok(Vec2::new(x as f32, y as f32)))?;
        ...
        for (entity, scripts) in pairs {
            for (slot, attachment) in scripts.scripts.iter().enumerate() {
                ...
                if !self.instances.contains_key(&key) {
                    let mut instance = self.create_instance(key, attachment, module_modified)?;
    ...
    fn capture_fn(&self, env: &mlua::Table<'_>, name: &str) -> Result<Option<mlua::RegistryKey>> {
...
fn parse_key(name: &str) -> Option<winit::keyboard::KeyCode> {
    ...
    match name {
        // Existing arrow / WASD / space
        "W" | "w" => Some(KeyCode::KeyW),
        "A" | "a" => Some(KeyCode::KeyA),
        "S" | "s" => Some(KeyCode::KeyS),
        "D" | "d" => Some(KeyCode::KeyD),
        "Space" | "space" => Some(KeyCode::Space),
        "Left" | "left" | "ArrowLeft" => Some(KeyCode::ArrowLeft),
        "Right" | "right" | "ArrowRight" => Some(KeyCode::ArrowRight),
        "Up" | "up" | "ArrowUp" => Some(KeyCode::ArrowUp),
        "Down" | "down" | "ArrowDown" => Some(KeyCode::ArrowDown),
