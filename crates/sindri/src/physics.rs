...
impl PhysicsWorld {
    ...
    pub fn add_collider_with_material(
        ...
        self.add_collider_with_material_handle(
            entity,
            shape,
            offset,
            density,
    ...
    pub(crate) fn add_collider_with_material_handle(
        &mut self,
        entity: EntityId,
        shape: ColliderShape,
        offset: Vec2,
    ...
    pub fn add_sensor(
        ...
        self.add_sensor_handle(entity, shape, offset)?;
        Ok(())
    }

    pub(crate) fn add_sensor_handle(
        &mut self,
        entity: EntityId,
        shape: ColliderShape,
        offset: Vec2,
    ...
    /// the set of groups this entity can collide with. If the entity has no body,
    ...
    /// Put an entity on one collision layer and allow it to collide with everything.
    pub fn set_collision_layer(&mut self, entity: EntityId, layer: u8) {
        let Some(memberships) = layer_mask(layer) else {
            return;
        };
        self.set_collision_groups(entity, memberships, u32::MAX);
    }

    /// Put an entity on one collision layer and set the layers it collides with.
    pub fn set_collision_mask(&mut self, entity: EntityId, layer: u8, mask: u32) {
        let Some(memberships) = layer_mask(layer) else {
            return;
        };
        self.set_collision_groups(entity, memberships, mask);
    }

    pub(crate) fn set_collider_friction(&mut self, collider: ColliderHandle, friction: f32) {
        if let Some(collider) = self.colliders.get_mut(collider) {
            collider.set_friction(friction);
        }
    }

    pub(crate) fn set_collider_restitution(&mut self, collider: ColliderHandle, restitution: f32) {
        if let Some(collider) = self.colliders.get_mut(collider) {
            collider.set_restitution(restitution);
        }
    }

    pub(crate) fn set_collider_sensor(&mut self, collider: ColliderHandle, sensor: bool) {
        if let Some(collider) = self.colliders.get_mut(collider) {
    ...
    /// Write each physics body's current position and rotation back to the
    /// entity's [`Transform`] component in `world`.
    ///
    /// Call this once per fixed-update step, after [`PhysicsWorld::step`],
    /// to keep `Transform` in sync with the physics simulation.
    /// Entities that have a body but no `Transform` component are silently skipped.
    ///
    /// # Example
    ///
    /// ```ignore
    /// fn fixed_update(&mut self, ctx: &mut EngineContext, dt: f32) {
    ///     ctx.physics.step(dt);
    ///     ctx.physics.sync_transforms(&mut ctx.world);
    /// }
    /// ```
...
fn layer_mask(layer: u8) -> Option<u32> {
    if layer < 32 {
        Some(1u32 << layer)
    } else {
        None
