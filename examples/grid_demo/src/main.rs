...
impl GridDemo {
    ...
    fn find_path_grid(&self, start: GridCoord, goal: GridCoord) -> Option<Vec<GridCoord>> {
        ...
        if let Some(path_nodes) =
            AStarPathfinder::find_path_grid(&self.pathfinding_grid, start_node, goal_node)
        {
            // Convert back to GridCoord
            Some(
                path_nodes
                    .iter()
                    .map(|n| GridCoord::new(n.x, n.y))
                    .collect(),
            )
        } else {
            None
...
impl Game for GridDemo {
    ...
    fn update(&mut self, ctx: &mut sindri::EngineContext) -> Result<()> {
        ...
        if input.is_mouse_pressed(sindri::MouseButton::Left) {
            if self.grid.is_valid(&mouse_grid)
                && self.grid.get(mouse_grid).copied().unwrap_or(false)
            {
        ...
        if self.agent_target_grid.is_some() {
            if self.agent_path_index < self.agent_path.len() {
                ...
                self.agent_world_pos = self
                    .agent_world_pos
                    .lerp(next_world, t * t * (3.0 - 2.0 * t));
    ...
    fn draw(&mut self, ctx: &mut sindri::EngineContext) -> Result<()> {
        ...
        if let Some(font) = self.font {
            ...
            self.hud.add_text(HudText {
                text: format!(
                    "Agent Grid: ({}, {})",
                    self.agent_grid_pos.x, self.agent_grid_pos.y
                ),
                font,
                size: 16.0,
...
fn main() -> Result<()> {
    Engine::new()
        .with_title("Sindri Grid-Based Movement Demo")
        .with_size(1280, 720)
        .with_vsync(true)
        .run(GridDemo::new())
}
